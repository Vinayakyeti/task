...
export const createOperation = async (incomingArgs)=>{
    ...
    try {
        ...
        const { autosave = false, collection: { config: collectionConfig }, collection, depth, disableVerificationEmail, draft = false, duplicateFromID, overrideAccess, overwriteExistingFiles = false, populate, req: { fallbackLocale, locale, payload, payload: { config } }, req, select: incomingSelect, showHiddenFields } = args;
        let { data } = args;
