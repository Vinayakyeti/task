...
export const generatePageMetadata = async ({
  ...
  switch (segments.length) {
    case 0:
      {
        meta = await generateDashboardViewMetadata({
          config,
          i18n
        });
        break;
    ...
    case 1:
      {
        if (oneSegmentMeta[segmentOne] && segmentOne !== 'account') {
          ...
          // --> /account
          meta = await generateAccountViewMetadata({
            config,
            i18n
          });
          break;
    ...
    case 2:
      {
        if (`/${segmentOne}` === config.admin.routes.reset) {
          // --> /reset/:token
          meta = await generateResetPasswordViewMetadata({
            config,
            i18n
          });
        }
        if (isCollection) {
          ...
          meta = await generateListViewMetadata({
            collectionConfig,
            config,
            i18n
          });
    ...
    default:
      {
        if (segmentTwo === 'verify') {
          ...
          meta = await generateVerifyViewMetadata({
            config,
            i18n
          });
  ...
  if (!meta) {
    ...
    if (viewKey) {
      ...
      meta = await generateNotFoundViewMetadata({
        config,
        i18n
      });
    }
